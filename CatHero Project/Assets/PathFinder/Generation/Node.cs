...
namespace K_PathFinder.NodesNameSpace {
    //abstract
    public abstract class NodeAbstract : ICellContentValue {
    ...
    public class NodeTemp : NodeAbstract {
        ...
        /// </summary>
        public EdgeTemp this[int layer, int hash] {
            get {
                VectorInt.Vector2Int key = new VectorInt.Vector2Int(layer, hash);
                EdgeTemp result;

                if (edges.TryGetValue(key, out result) == false) {
                    result = new EdgeTemp(this, layer, hash);  
                    edges[key] = result;
                }

                return result;
            }
            set { edges[new VectorInt.Vector2Int(layer, hash)] = value; }
        }
        #endregion
