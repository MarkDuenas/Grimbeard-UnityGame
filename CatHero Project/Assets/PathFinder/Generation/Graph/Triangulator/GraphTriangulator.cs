...
namespace K_PathFinder.GraphGeneration {
    public class GraphTriangulator {
        ...
        public GraphTriangulator(GraphGeneratorNew generator, NavMeshTemplateCreation template) {
            ...
            var genNodes = generator.nodesArray;
            int genNodesCreated = generator.nodesCreated;

            var genEdges = generator.edgesArray;
            int genEdgesCount = generator.edgesCreated;

            Node[] tempNodeArray = new Node[genNodesCreated];
            EdgeSimple[] tempEdgeArray = new EdgeSimple[genEdgesCount];

            for (int i = 0; i < genNodesCreated; i++) {
                var nodeInfo = genNodes[i];
                if (nodeInfo.ID != INVALID_VALUE) {
                    Node n = new Node(nodeInfo.positionV3);
                    tempNodeArray[i] = n;
                }
            }

            for (int i = 0; i < genEdgesCount; i++) {
                var edgeInfo = genEdges[i];

                if (edgeInfo.ID != INVALID_VALUE & 
                    genNodes[edgeInfo.nodeLeft].ID != INVALID_VALUE &
                    genNodes[edgeInfo.nodeRight].ID != INVALID_VALUE) {
                    EdgeSimple e = new EdgeSimple(tempNodeArray[edgeInfo.nodeLeft], tempNodeArray[edgeInfo.nodeRight]);
                    tempEdgeArray[edgeInfo.ID] = e;

                    var curDicNodes = dicNodes[edgeInfo.layer];
                    var curDicEdges = dicEdges[edgeInfo.layer];
                    var curDicValues = dicValues[edgeInfo.layer];

                    if (curDicNodes.ContainsKey(edgeInfo.hash) == false) {
                        curDicNodes.Add(edgeInfo.hash, new List<NodeAbstract>());
                        curDicEdges.Add(edgeInfo.hash, new List<EdgeAbstract>());
                        curDicValues.Add(edgeInfo.hash, new Dictionary<NodeAbstract, TriangulatorNodeData>());
                    }

                    curDicNodes[edgeInfo.hash].Add(e.a);
                    curDicEdges[edgeInfo.hash].Add(e);

                    int nodeID_1 = edgeInfo.nodeLeft;
                    int nodeID_2 = edgeInfo.nodeRight;
                    int nodeID_3 = generator.GetConnectedNodeNew(nodeID_2, edgeInfo.layer, edgeInfo.hash);

                    var node_1 = genNodes[nodeID_1];
                    var node_2 = genNodes[nodeID_2];
                    var node_3 = genNodes[nodeID_3];

                    float cross = SomeMath.V2Cross(
                        node_3.x - node_2.x, node_3.z - node_2.z,
                        node_1.x - node_2.x, node_1.z - node_2.z);

                    //Debuger_K.AddLabel(node_2.positionV3, cross);

                    if (cross < 0) {
                        Vector2 directionLast = new Vector2(node_3.x - node_2.x, node_3.z - node_2.z).normalized;
                        Vector2 directionFirst = new Vector2(node_1.x - node_2.x, node_1.z - node_2.z).normalized;
                        curDicValues[edgeInfo.hash].Add(tempNodeArray[nodeID_2], new TriangulatorNodeData(cross, Vector2.Angle(directionLast, directionFirst), (directionLast + directionFirst).normalized * -1));
                    }
                    else
                        curDicValues[edgeInfo.hash].Add(tempNodeArray[nodeID_2], new TriangulatorNodeData(cross, 0, Vector2.zero));
                }
            }
            ...
            for (int id = 0; id < maxLayers; id++) {
                foreach (var hash in dicNodes[id].Keys) {
                    ...
                    List<NodeAbstract> setNodes = dicNodes[id][hash];
                    List<EdgeAbstract> setEdges = dicEdges[id][hash];
                    Dictionary<NodeAbstract, TriangulatorNodeData> setValues = dicValues[id][hash];

                    data.Add(new TriangulatorDataSet(template, setNodes, setEdges, setValues, id, area, pass));
                }
            }
        ...
        public void Triangulate(ref GraphCombiner combiner, NavMeshTemplateCreation template) {
#if UNITY_EDITOR
            //for debug
            this.pos = template.gridPosition;
            ...
            if (Debuger_K.doDebug && Debuger_K.debugOnlyNavMesh == false)
                DebugDataSets(template.gridPosition, properties);
#endif
