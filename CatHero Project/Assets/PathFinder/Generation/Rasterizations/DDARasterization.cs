...
namespace K_PathFinder.Graphs {
    public static class DDARasterization {
        ...
        /// <summary>
        /// DONT FORGET TO SCALE ALL SO ONE PIXEL IS 1f
        /// this have input with bool[] map and draw lines on it
        /// copy pase from  public static void DrawLine(float x0float, float y0float, float x1float, float y1float, DelegateDDA1 del)
        /// index is (y * gridSize) + x
        /// </summary>
        public static void DrawLine(float x0float, float y0float, float x1float, float y1float, bool[] map, int gridSize) {
            int x0int = (int)x0float;
            int y0int = (int)y0float;
            int x1int = (int)x1float;
            int y1int = (int)y1float;
            ...
            if (x0int == x1int) {
                if (y0int == y1int) {//inside one pixel
                    ...
                    tempY = y0int;
                    if (tempX < 0) tempX = 0;
                    else if (tempX >= gridSize) tempX = gridSize - 1;
                    if (tempY < 0) tempY = 0;
                    else if (tempY >= gridSize) tempY = gridSize - 1;

                    map[(tempY * gridSize) + tempX] = true;
                    return;
                }
                //along x axis
                if (y0int < y1int) {
        ...
        public static void DrawLine(float x0float, float y0float, float x1float, float y1float, float pixelSize, bool[] map, int gridSize) {
            DrawLine(x0float / pixelSize, y0float / pixelSize, x1float / pixelSize, y1float / pixelSize, map, gridSize);
        }
        ...
        /// <summary>
        /// result of rasterization is in tempMap[y * gridSize + x] 
        /// </summary>
        public static void Rasterize(bool[] tempMap, int gridSize, float pixelSize, float ax, float ay, float bx, float by, float cx, float cy, float gridOffsetX, float gridOffsetY) {
            Rasterize(tempMap, gridSize, pixelSize, ax - gridOffsetX, ay - gridOffsetY, bx - gridOffsetX, by - gridOffsetY, cx - gridOffsetX, cy - gridOffsetY);
        }
        ...
        /// result of rasterization is in tempMap[y * gridSize + x] 
        /// </summary>
        public static void Rasterize(bool[] grid, int gridSize, float ax, float ay, float bx, float by, float cx, float cy) {
            ...
            //fixing if all positions inside one pixel
            if (ax > 0 & ax < maxSize & ay > 0 & ay < maxSize)
                grid[(int)ay * gridSize + (int)ax] = true;
            if (bx > 0 & bx < maxSize & by > 0 & by < maxSize)
                grid[(int)by * gridSize + (int)bx] = true;
            if (cx > 0 & cx < maxSize & cy > 0 & cy < maxSize)
                grid[(int)cy * gridSize + (int)cx] = true;

            //DelegateDDA1 del = (x, y) => {
            //    grid[Mathf.Clamp(y, 0, gridSize - 1) * gridSize + Mathf.Clamp(x, 0, gridSize - 1)] = true;
            //};

            DrawLine(ax, ay, bx, by, grid, gridSize);
            DrawLine(bx, by, cx, cy, grid, gridSize);
            DrawLine(cx, cy, ax, ay, grid, gridSize);
