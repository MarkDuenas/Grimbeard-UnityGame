...
namespace K_PathFinder.Rasterization {
    public class CSRasterization2D {
        ...
        public CSRasterization2DResult Rasterize(Vector3[] verts, int[] tris, int trisLength, int sizeX, int sizeZ, float chunkPosX, float chunkPosZ, float voxelSize, float maxSlopeCos, bool debug = false) {
            if (trisLength == 0)
                return null;

            int dsLength = trisLength / 3;
                ...
                foreach (var dataSet in ds) {
                    ...
                    for (int x = dataSet.minX; x < dataSet.maxX; x++) {
                        for (int z = dataSet.minZ; z < dataSet.maxZ; z++) {
                            ...
                            if (SomeMath.PointInTriangleClockwise(A.x, A.z, B.x, B.z, C.x, C.z, pointX, pointZ)) {
                                float height = SomeMath.CalculateHeight(A, B, C, pointX, pointZ);
                                Debuger_K.AddDot(new Vector3((x * voxelSize) + chunkPosX, height, (z * voxelSize) + chunkPosZ));
