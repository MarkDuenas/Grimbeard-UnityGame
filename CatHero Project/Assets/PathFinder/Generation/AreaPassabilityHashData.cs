...
namespace K_PathFinder {
    ...
    public class AreaPassabilityHashData {
        private HashSet<Area> _areaPool = new HashSet<Area>();
        ...
        public int defaultAreaIndexInt;
        public byte defaultAreaIndexByte;
        ...
        public void AssignData() {
            areaByIndex = new Area[_areaPool.Count];
            int a = 0;
            foreach (var item in _areaPool) {
                areaByIndex[a++] = item;
            }

            areaByIndex = areaByIndex.OrderBy(x => x.overridePriority).ToArray();
            
            //for (int i = 0; i < areaByIndex.Length; i++) {
            //    Debug.Log(areaByIndex[i].name + " : " + areaByIndex[i].overridePriority);
            //}
            ...
            defaultAreaIndexByte = areaToIndex[PathFinder.getDefaultArea];
            defaultAreaIndexInt = defaultAreaIndexByte;
        }
