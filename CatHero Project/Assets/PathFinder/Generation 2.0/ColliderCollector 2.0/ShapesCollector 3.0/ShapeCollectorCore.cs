...
namespace K_PathFinder.Collector {
    public partial class ShapeCollector {
        ...
        const int INITIAL_FREE_INDEX_POOL_SIZE = 128;
        //*********POOL*********//
        ...
        public void Init(NavMeshTemplateCreation template) {
            ...
            voxelDistanceThreshold = template.voxelBleedDistance;
        ...
        //function to add a single voxel to whole  
                ...
                while (true) {
                    ...
                    if (curNode.min > max) {
                        ...
                        if (freeIndex == arrayData.Length) 
                            GenericPoolArray<Data>.IncreaseSize(ref arrayData);                        
                    ...
                    float dif = max - curNode.max;
                    if (dif < 0) dif *= -1f;
                    if (dif < voxelDistanceThreshold) {
                        if (curNode.pass > pass)
                            pass = curNode.pass;
                    ...
                    if (val.pass != -1) {
                        ...
                        if (curNode.next == -2) {
                            ...
                            while (true) {
                                ...
                                float dif = val.max - curNode.max;
                                if (dif < 0) dif *= -1f;

                                //Debuger_K.AddLabel(GetPos(x, z, val.max), dif);

                                if (dif < voxelDistanceThreshold) {
                                    if (curNode.pass > val.pass)
                                        val.pass = curNode.pass;
                                    if (curNode.area > area)
                                        area = curNode.area;
                                }
        ...
        public void AppendCompactData(DataCompact[] value, byte[] area) {
            ...
            GenericPoolArray<DataCompact>.ReturnToPool(ref value);
        }
        ...
        private byte GetAreaValue(Area area) {
            //Debug.Log(area.name + " : " + template.hashData.areaToIndex[area]);
        ...
        public void MakeHole(List<ShapeDataAbstract> shapes, Area area) {
            ...
            if(appyArea)
                ...
                areaValue = template.hashData.defaultAreaIndexByte;
