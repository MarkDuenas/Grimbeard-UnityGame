...
namespace K_PathFinder.Collector {
    public partial class ShapeCollector {
        ...
        private void AppendSpherePrivate(DataCompact[] compactData, Vector3 spherePos, float sphereRadius, float heightDelta, bool appentPassive, bool isWalkable) {
            ...
            int xMin = (int)((sphereLocalX - sphereRadius) / voxelSize);
            if (xMin < 0) xMin = 0; else if (xMin > template.lengthX_extra) xMin = template.lengthX_extra;

            int xMax = (int)((sphereLocalX + sphereRadius) / voxelSize) + 1;
            if (xMax < 0) xMax = 0; else if (xMax > template.lengthX_extra) xMax = template.lengthX_extra;

            int zMin = (int)((sphereLocalZ - sphereRadius) / voxelSize);
            if (zMin < 0) zMin = 0; else if (zMin > template.lengthX_extra) zMin = template.lengthZ_extra;

            int zMax = (int)((sphereLocalZ + sphereRadius) / voxelSize) + 1;
            if (zMax < 0) zMax = 0; else if (zMax > template.lengthX_extra) zMax = template.lengthZ_extra;
