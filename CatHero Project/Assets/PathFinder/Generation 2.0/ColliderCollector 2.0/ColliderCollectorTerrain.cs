...
namespace K_PathFinder.Collector {
    public partial class ColliderCollector {
        ...
        private void CollectTerrainOnGPU(TerrainColliderInfoMesh terrain) {
            ...
            int[] tris;
            int trisLength;
            GetTerrainMesh(terrain, out verts, out tris, out trisLength);

            terrainGPUResuts.Add(new TerrainResultGPU() {
                result = PathFinder.sceneInstance.Rasterize2D(verts, tris, trisLength, vSizeX, vSizeZ, chunkPosX, chunkPosZ, template.voxelSize, maxSlopeCos),
                info = terrain
            });
        ...
        private void CollectTerrainGPU(ShapeCollector shape) {
            if (terrainGPUResuts.Count > 0) {
                if (profiler != null) profiler.AddLogFormat("Start terrain collecting by GPU. count: {0}", terrainGPUResuts.Count);
                ...
                foreach (var terrainGPU in terrainGPUResuts) {
                    ...
                    profiler.AddLog("Terrain Collected by GPU");
                }
            }
            else {
                if (profiler != null) profiler.AddLog("No pending terrain to be collected by GPU");
            }
        }

        private void CollectTerrainCPU(ShapeCollector shape) {
            if (terrainsInfoForCPU.Count > 0) {
                if (profiler != null) profiler.AddLogFormat("Start terrain collecting by CPU. count: {0}", terrainsInfoForCPU.Count);

                foreach (TerrainColliderInfoMesh terrainInfo in terrainsInfoForCPU) {
                    Vector3[] verts;
                    int[] tris;
                    int trisCount;
                    GetTerrainMesh(terrainInfo, out verts, out tris, out trisCount);

                    //shape.AppendTerrain(terrainInfo, PathFinder.getDefaultArea);

                    if (terrainInfo.alphaMap == null) {
                        shape.AppendTerrain(verts, tris, trisCount, PathFinder.getDefaultArea);
                    }
                    else {
                        byte[] areaSplatMap = Pool.GenericPoolArray<byte>.Take(shape.flattenSize);
                        GenerateTerrainAreaMapFromSplatMap(terrainInfo, shape, ref areaSplatMap);
                        shape.AppendTerrain(verts, tris, trisCount, areaSplatMap);
                        Pool.GenericPoolArray<byte>.ReturnToPool(ref areaSplatMap);
                    }
                    ...
                    if (profiler != null) profiler.AddLog("Terrain Collected by CPU");
                }
            }
            else {
                if (profiler != null) profiler.AddLog("No pending terrain to be collected by CPU");
            }
        ...
        private void AddColliderTerrain(Collider collider, TerrainCollectorType terrainCollectionType) {
            ...
            float minSize = PathFinderSettings.TERRAIN_FAST_MIN_SIZE;
            ...
            int heightMapSizeX = data.heightmapResolution;
            int heightMapSizeZ = data.heightmapResolution;
            ...
            if (navmeshSettings != null && navmeshSettings.isActiveAndEnabled && navmeshSettings.data.Any(x => x != 0)) { //0 is default area in global settings
        ...
        public int pendingTerrainCPU {
            get { return terrainsInfoForCPU.Count; }
        }

        public int pendingTerrainGPU {
            get { return terrainGPUResuts.Count; }
        }
        ...
        public static void GetTerrainMesh(TerrainColliderInfoMesh terrain, out Vector3[] verts, out int[] tris, out int trisLength) {
            ...
            trisLength = (hSizeX - 1) * (hSizeZ - 1) * 6;
        ...
        /// </summary>
        private void GenerateTerrainAreaMapFromSplatMap(TerrainColliderInfoMesh terrainInfo, ShapeCollector shape, ref byte[] result) {
            ...
            int[] data = terrainInfo.settings.data; //transfer data arrat to array that contains correct values
            int[] a2d = new int[data.Length];
            for (int i = 0; i < data.Length; i++) {
                a2d[i] = template.hashData.areaToIndex[PathFinder.GetArea(data[i])];
            }
            ...
            for (int x = startXClamp; x < endXClamp; x++) {
                int curX = Mathf.RoundToInt((x - terrainStartX) / terrainSizeX * alphaWidth) - alphaStartX;
                if (curX < 0)
                    curX = 0;
                else if (curX > alphaSizeXIndexLimit)
                    curX = alphaSizeXIndexLimit;


                for (int z = startZClamp; z < endZClamp; z++) {
                    int curZ = Mathf.RoundToInt((z - terrainStartZ) / terrainSizeZ * alphaHeight) - alphaStartZ;
                    if (curZ < 0)
                        curZ = 0;
                    else if (curZ > alphaSizeZIndexLimit)
                        curZ = alphaSizeZIndexLimit;
                    ...
                    //Debug.LogFormat("x {0} z {1} h {2} v {3}", x, z, highest, template.hashData.areaByIndex[(byte)a2d[highest]].name);
                    ...
                    result[((z - template.startZ_extra) * shapeSizeX) + (x - template.startX_extra)] = (byte)a2d[highest];                  
                }
            }
        }
    }
