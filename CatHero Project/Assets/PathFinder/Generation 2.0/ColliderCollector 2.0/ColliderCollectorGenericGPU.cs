...
namespace K_PathFinder.Collector {
    public partial class ColliderCollector {
        ...
        private struct ComputeShaderResultHolder {     
            ...
            public ComputeShaderResultHolder(CSRasterization3DResult result, Area area) {    
                this.result = result;
                this.area = area;
            }
        }
        ...
        //results
        public List<KeyValuePair<AreaWorldMod, ShapeCollector>> shapeCollectorGenericGPU_mods;
        ...
        private void AddColliderGenericGPU(Collider collider) {
            ...
            if(verts != null & tris != null) {
                ...
                if (result != null)
                    collectedComputeShaderData.Add(new ComputeShaderResultHolder(result, area));
            }
        }

        private void CollectCollidersGPU(ShapeCollector collector) {
            if (collectedComputeShaderData.Count > 0) {
                ...
                if (profiler != null) profiler.AddLogFormat("End shape collecting by GPU. Added {0} shapes", collectedComputeShaderData.Count);
            }
            else {
                if (profiler != null) profiler.AddLog("No pending shapes to be collected by GPU");
            }
        }

        public int pendingGenericShapeGPU {
            get { return collectedComputeShaderData.Count; }
        }
    }
}