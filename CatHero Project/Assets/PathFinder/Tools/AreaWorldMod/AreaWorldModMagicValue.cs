...
namespace K_PathFinder {
    ...
    public class AreaWorldModMagicValue {
        ...
        public void RecalculateBounds() {
            Matrix4x4 G2L = container.threadSafeLocalToWorldMatrix;
            ...
            switch (myType) {
                case AreaWorldModMagicValueType.Cuboid:
                    ...
                    break;
                case AreaWorldModMagicValueType.Sphere:
                    bounds = new Bounds(container.threadSafePosition + (container.threadSafeRotation * position), new Vector3(radius * 2, radius * 2, radius * 2));
                    break;
                case AreaWorldModMagicValueType.Capsule:
                    Vector3 capsulePos = G2L.MultiplyPoint(position);
                    Vector3 posLow = capsulePos + (BC.GetRotation(G2L) * BC.GetRotation(L2W) * new Vector3(0, (-height * 0.5f) + radius, 0));
                    Vector3 posHigh = capsulePos + (BC.GetRotation(G2L) * BC.GetRotation(L2W) * new Vector3(0, (height * 0.5f) - radius, 0));
        ...
        public void DrawGizmos(float alpha) {
            Matrix4x4 matrixG2L = container.threadSafeLocalToWorldMatrix;
            Gizmos.color = SetColorAlpha(Gizmos.color, alpha);
            switch (myType) {
                case AreaWorldModMagicValueType.Cuboid:
                    Gizmos.DrawMesh(Shapes.cubeMesh, matrixG2L.MultiplyPoint(position), BC.GetRotation(matrixG2L) * rotation, cubeSize * 2);
                    break;
                case AreaWorldModMagicValueType.Sphere:             
                ...
                case AreaWorldModMagicValueType.Capsule:               
                    if (capsuleMesh != null && capsuleMesh.vertices.Length != 0)
                        Gizmos.DrawMesh(capsuleMesh, matrixG2L.MultiplyPoint(position), BC.GetRotation(matrixG2L) * rotation, Vector3.one);
                    break;
            }
        }
