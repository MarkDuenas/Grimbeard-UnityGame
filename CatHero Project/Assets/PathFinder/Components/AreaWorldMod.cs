...
namespace K_PathFinder {
    ...
    public class AreaWorldMod : MonoBehaviour, IChunkContent {
        ...
        [SerializeField][LayerPF] private int _layer = 0;
        ...
        private bool _enabledThreadSafe;
        private string _threadSafeTag;
        private int _threadSafeUnityLayer;
        private Vector3 _threadSafePosition;
        private Quaternion _threadSafeRotation;
        ...
        public void OnPathFinderSceneInit() {
            UpdateThreadSafeValues();
            Init();
            RebuildChilds(false);
            DirtyAll();
            CheckDirty();
            PathFinder.ProcessChunkContent(this);
            lock (this)
                _enabledThreadSafe = true;
        }
        
        public void OnDisable() {
            PathFinder.RemoveChunkContent(this);
            lock (this)
                _enabledThreadSafe = false;
        }
        ...
        public void UpdateThreadSafeValues() {
            lock (this) {
                _threadSafeTag = tag;
                _threadSafeUnityLayer = gameObject.layer;
                _threadSafePosition = transform.position;
                _threadSafeRotation = transform.rotation;
            }
        }
        ...
        /// If cells can be used then it can be used by standart search.
        /// If not then it will be excluded from standart search.
        /// </summary>
        ...
        /// <summary>
        /// Sets particular layer for this area
        /// </summary>
        public void SetCellsLayer(int targetLayer) {
            if (_useAdvancedArea == false)
                Debug.LogWarningFormat("You trying to change cell state when this AreaWorldMod is not setted up for advanced area. Position {0}", transform.position);

            if (_layer == targetLayer)
                return;
            _layer = targetLayer;

            PathFinder.AddPathfinderThreadDelegate(() => {
                foreach (var cell in advancedArea.cells) {
                    cell.SetBitMaskLayer(targetLayer);
                }
            });
        }
        /// <summary>
        /// Sets particular layer for this area
        /// </summary>
        public void SetCellsLayer(int targetLayer, params AgentProperties[] targets) {
            if (_useAdvancedArea == false)
                Debug.LogWarningFormat("You trying to change cell state when this AreaWorldMod is not setted up for advanced area. Position {0}", transform.position);
            ...
            PathFinder.AddPathfinderThreadDelegate(() => {
                foreach (var cell in advancedArea.cells) {
                    if (targets.Contains(cell.graph.properties)) {
                        cell.SetBitMaskLayer(pathFinderLayer);
                    }
                }
            });
        }
        ...
        public bool enabledThreadsafe {
            get {
                lock (this)
                    return _enabledThreadSafe;
            }
        }

        public string threadsafeTag {
            get {
                lock (this)
                    return _threadSafeTag;
            }
        }

        public int threadsafeUnityLayer {
            get {
                lock (this)
                    return _threadSafeUnityLayer;
            }
        }
        ...
        public Vector3 threadSafePosition {
            get {
                lock (this)
                    return _threadSafePosition;
            }
        }
        public Quaternion threadSafeRotation {
            get {
                lock (this)
                    return _threadSafeRotation;
            }
        }
        public Matrix4x4 threadSafeLocalToWorldMatrix {
            get { return Matrix4x4.TRS(threadSafePosition, threadSafeRotation, Vector3.one); }
        }
        public Matrix4x4 ThreadSafeWorldToLocalMatrix {
            get { return threadSafeLocalToWorldMatrix.inverse; }
        }
...
#if UNITY_EDITOR
        [MenuItem(PathFinderSettings.UNITY_TOP_MENU_FOLDER + "/Create Area Modifier", false, 2)]
        public static void Create() {
            GameObject go = new GameObject("Area Modifier");
            Undo.RegisterCreatedObjectUndo(go, "Created Area Modifier");
            AreaWorldMod am = go.AddComponent<AreaWorldMod>();
            am.OnPathFinderSceneInit();
        }
#endif
    }
}